...
check_proxy() {
  ...
  npm config set proxy http://192.168.1.19:10809
  npm config set https-proxy http://192.168.1.19:10809
...
install_libsrtp2() {
  ...
  if [ -d $LIB_DIR ]; then
    cd $LIB_DIR
    rm -rf libsrtp-2.1.0
    curl -x socks5://192.168.1.19:10808 -o libsrtp-2.1.0.tar.gz https://codeload.github.com/cisco/libsrtp/tar.gz/v2.1.0
